use std::env;
use auth_api::services::crypto::{generate_jwt, verify_jwt};
...
fn test_generate_jwt_success() {
    ...
    let jwt_result = generate_jwt(email);
...
fn test_generate_jwt_empty_email() {
    ...
    let empty_email = "";
    let jwt_result = generate_jwt(empty_email);
...
fn test_verify_jwt_success() {
    ...
    let email = "test@example.com";
    let valid_token = generate_jwt(email);
    assert!(valid_token.is_ok());

    let result = verify_jwt(valid_token.unwrap().as_str());
    assert!(result.is_ok());
...
fn test_verify_jwt_empty_token() {
    env::set_var("JWT_SECRET", "valid_secret");
    let empty_token = "";
    let result = verify_jwt(empty_token);
